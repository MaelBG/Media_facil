...
export default function App() {
  ...
  const getStudentVistoScore = (alunoId) => {
    ...
    const totalVistos = vistos.filter(v => v.aluno_id === alunoId && v.class_id === selectedClassId && v.status === true).length;
  ...
  const getStudentVistoCount = (alunoId) => {
    return vistos.filter(v => v.aluno_id === alunoId && v.class_id === selectedClassId && v.status === true).length;
  };
  ...
  const _unused_getStudentDashboardWeightedAverage_original = () => {
    ...
    if (provas.length > 0) {
      ...
      let somaMaxima = 0;
      provas.forEach(n => {
        somaObtida += n.valor_obtido;
        somaMaxima += n.valor_maximo;
      });
    ...
    // Vistos
    let vScore = 10.0;
    if (studentReport.vistos.length > 0) {
      const concluidos = studentReport.vistos.filter(v => v.status === true).length;
      vScore = (concluidos / studentReport.vistos.length) * 10;
    }

    const pesos = studentReport.turma.pesos || { provas: 50, prova_paulista: 20, atividades: 15, vistos: 15 };
    const wProvas = pesos.provas ?? 50;
    const wPaulista = pesos.prova_paulista ?? 20;
    const wAtividades = pesos.atividades ?? 15;
    const wVistos = pesos.vistos ?? 15;

    const finalAvg = (pAvg * wProvas + paulistaAvg * wPaulista + aScore * wAtividades + vScore * wVistos) / 100;
    return finalAvg.toFixed(1);
  };
          ...
          <nav className="space-y-4">
            ...
            {classes && classes.length > 0 && (
              <div className="space-y-1.5 px-1">
                ...
                <select
                  value={view === "teacher_class" ? selectedClassId || "" : ""}
                  onChange={(e) => {
                    if (e.target.value) {
                      setSelectedClassId(e.target.value);
                      loadClassData(e.target.value);
                      setActiveTab("boletim");
                      setView("teacher_class");
                    }
                  }}
                  className="w-full p-2.5 bg-surface-container-high text-on-surface border border-outline-variant rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/50 outline-none transition-all duration-200 cursor-pointer"
                >
          ...
          <div className="space-y-2 pt-2 border-t border-outline-variant/40">
            ...
            <div className="space-y-1">
              ...
              <a 
                ...
                onClick={(e) => { 
                  e.preventDefault(); 
                  alert("Calendário Acadêmico:\nSemestre Letivo 2026.1 - Lançamento de notas bimestrais e visto de cadernos."); 
                }}
                className="flex items-center gap-2 px-2.5 py-2 text-xs text-on-surface-variant hover:text-primary hover:bg-surface-container-high rounded-lg transition-all"
              >
        ...
        <div className="border-t border-outline-variant pt-4 space-y-1">
          <div 
            onClick={async () => {
              ...
              setProfileEscola(currentUser?.escola || "Etec de Vila Carrão");
              setProfileSemestre(currentUser?.semestre || "Semestre 2026.1");
        ...
        <div className="flex">
          ...
          <aside className="w-64 fixed left-0 top-0 h-screen bg-surface-container-low border-r border-outline-variant p-4 flex flex-col justify-between">
            <div className="space-y-6">
              ...
              <nav className="space-y-4">
                <button className="w-full flex items-center gap-3 p-3 bg-secondary-container text-on-secondary-container rounded-xl font-bold transition-all text-left text-sm">
                ...
                {studentReport?.materias && studentReport.materias.length > 0 && (
                  <div className="space-y-1.5 px-1">
                    ...
                    <select
                      value={studentReport?.activeClassId || ""}
                      onChange={(e) => loadStudentData(currentUser.id, e.target.value)}
                      className="w-full p-2.5 bg-surface-container-high text-on-surface border border-outline-variant rounded-xl text-xs font-semibold focus:ring-2 focus:ring-secondary/50 outline-none transition-all duration-200 cursor-pointer"
                    >
