...
export default function StudentProfile({ currentUser, setCurrentUser, navigateTo }) {
  ...
  const [profileAvatarCor, setProfileAvatarCor] = useState("bg-secondary");
  ...
  useEffect(() => {
    const initProfileForm = async () => {
      ...
      setProfileAvatarCor(currentUser.avatar_cor || "bg-secondary");
        ...
        <form onSubmit={handleUpdateProfile} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-6 py-2.5 bg-secondary hover:bg-secondary/95 text-white rounded-xl font-bold transition-all shadow-md active:scale-95 text-sm cursor-pointer disabled:opacity-55"
            >
